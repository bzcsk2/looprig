...
**增量式 JSON 验证与 Eager Dispatch**：
为防止模型在正文中输出正则或代码片段 `{ ... }` 导致假闭合的毁灭性后果，对流式 buffer 进行**增量式 JSON 验证**。仅当累积的 buffer 通过验证确认为结构完整的合法 JSON 参数时，才立即创建 Promise 发起 Eager Dispatch。具体实现待技术选型：可采用状态机驱动的流式 JSON 部分解析器，或对 buffer 进行安全的 `try-parse` 配合括号深度追踪（前者更可靠但实现复杂度更高，视实际假闭合频率决定）。
...
- **R1 适配**：对于 R1 模型的特有字段 `reasoning_content`，提取后通过事件流推给 TUI 渲染（可折叠展示），同时写入 assistant 历史消息保留多轮连续性。但 `reasoning_content` **不计入 ImmutablePrefix 的 cacheKey 计算**，因此不影响 prefix-cache 稳定性。
...
| JSON 假闭合防御失灵 | 中 | 高 | 当前稳定优先策略（完整 tool call 结束后执行工具）已规避假闭合问题；后续改为 eager dispatch 时需重新评估，结合正则底线判定，失败退回 Scavenge 阶段 |
