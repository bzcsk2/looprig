...
- **R1 适配**：对于 R1 模型的特有字段 `reasoning_content`，提取后通过事件流推给 TUI 渲染（可折叠展示），**不写入 AppendOnlyLog 上下文，不进入 API 请求**。reasoning 内容仅用于用户理解模型思考过程，不占用 token 窗口、不影响 prefix-cache、不参与后续轮次的推理。
