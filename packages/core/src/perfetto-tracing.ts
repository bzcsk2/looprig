/**
 * Simplified Perfetto Tracing for Deepicode
 *
 * Outputs Chrome Trace Event JSON format viewable in ui.perfetto.dev
 *
 * Enable via DEEPICODE_TRACE=1 or --trace CLI flag
 * Output: .deepicode/traces/trace-<session-id>.json
 */
...
export function initializePerfettoTracing(sid: string): void {
  const envValue = process.env.DEEPICODE_TRACE
  ...
  tracePath = join(process.cwd(), ".deepicode", "traces", `trace-${sid}.json`)
