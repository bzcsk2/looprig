...
const DEEPICODE_TASK_PREFIX = "Deepicode-"
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", () => {
      ...
      for (const line of lines) {
        ...
        if (!taskName.startsWith(DEEPICODE_TASK_PREFIX)) continue
        const name = taskName.slice(DEEPICODE_TASK_PREFIX.length)
...
async function createSchTaskJob(
  ...
  const sanitizedCommand = command.replace(/[\n\r]/g, " ")
  const sanitizedName = name.replace(/[\n\r]/g, "_")
  const taskName = `${DEEPICODE_TASK_PREFIX}${sanitizedName}`
