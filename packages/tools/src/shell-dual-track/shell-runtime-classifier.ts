 ...
 * - `long`  → 默认后台启动，hard timeout 给 24h
 * - `auto`  → 前台启动，超过 {@link SOFT_TIMEOUT_MS} 仍在跑则升级到后台
 *
 * 设计原则：零配置 — 全部白名单写死 const，不读 JSON / env / 命令行参数。
 */

/** 前台软超时：到达此时长仍在跑则 escalate 到后台。 */
export const SOFT_TIMEOUT_MS = 8_000

/** 后台默认 hard timeout — 5 分钟，兼容显式 background:true。 */
...
/** Harness / chat-ws 推送后台摘要的最小间隔 — 5 分钟。 */
 ...
 *
 * 优先级：
 * 1. classifier === `long` → 24h
 * 2. explicit === background → 5min（兼容显式 background:true）
 * 3. 其它 → 5min
 *
 * @param cls 分类结果
 * @param options 显式后台等选项
 */
 ...
 *
 * @param cls 分类结果
 * @param argsTimeout 用户显式 timeout
 * @param defaultTimeoutMs 默认前台 timeout
 */
