#!/usr/bin/env python3
"""
OpenCode Zen API 测试脚本

测试 opencode.ai 提供的免费 DeepSeek V4 Flash 模型。
无需注册，直接使用。
...
环境变量:
  ZEN_MODEL       模型名（默认 deepseek-v4-flash-free）
"""
...
def test_thinking_mode():
    section("思考模式")
    print("\n  --- a: 默认（Zen 默认返回 reasoning_content）---")
    body = {
        "model": MODEL,
    ...
    print(f"  reasoning_content: {'有' if msg2.get('reasoning_content') else '无'}")
    assert not msg2.get("reasoning_content"), "禁用思考模式不应返回 reasoning_content"
    print("  ✅ 通过")

    print("\n  --- c: reasoning_effort=max ---")
    body3 = dict(body)
    body3["thinking"] = {"type": "enabled"}
    body3["reasoning_effort"] = "max"
...
def test_multi_turn():
    ...
    body = {"model": MODEL, "messages": messages, "thinking": {"type": "enabled"}, "stream": False}
    t0 = time.time()
    resp = req("chat/completions", body)
